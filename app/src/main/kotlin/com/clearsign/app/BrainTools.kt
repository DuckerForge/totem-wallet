...
object BrainTools {
    ...
    val schema: JSONArray = JSONArray()
        ...
        .put(
            tool(
                "send_sol",
                "Propose sending SOL from the budget. Totem decides: it may sign silently, ask the person for a fingerprint, or refuse. A refusal is final — explain it to the user, never retry the same thing another way.",
                JSONObject()
        ...
        .put(
            tool(
                "market_search",
                "Look up a coin by name, symbol or mint address. Returns what the project actually is, its market cap rank, how it moved over 24h, a week, a month and a year, how much liquidity it has, whether Jupiter verified it, and Totem\u2019s own safety grade with the reasons. A missing \"about\" means nobody has listed it, which for a brand new coin is normal and for an old one is worth saying. Use it before proposing any swap into something the user does not already hold, and tell them what you found.",
    ...
    private suspend fun judge(ctx: Context, tx: ByteArray, intent: JSONObject, agent: String, ultraRequestId: String? = null): JSONObject {
        ...
        return verdict.toJson().put(
            "what_it_means",
            when (verdict.code) {
                "signed_silently" -> "Inside the rules: Totem signed and sent it. Tell the user it is done and give the signature."
