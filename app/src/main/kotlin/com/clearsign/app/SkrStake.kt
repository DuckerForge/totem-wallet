...
object SkrStake {
    ...
    data class Position(val guardian: String, val shares: BigInteger, val rawSkr: Long, val totalStakedRaw: Long) {
        val ui: Double get() = rawSkr / 1e6
        /**
         * What staking pays, as a yearly rate: the inflation goes to stakers only,
         * so it is the inflation on the whole supply spread over what is staked.
         * The published schedule: 10% in the first year, a quarter less each year
         * after (from the 21st of January 2026).
         */
        fun aprPct(supplyRaw: Long, now: Long = System.currentTimeMillis()): Double? {
            if (totalStakedRaw <= 0 || supplyRaw <= 0) return null
            val years = ((now - 1_768_953_600_000L) / 31_557_600_000.0).coerceAtLeast(0.0).toInt()
            val inflation = 0.10 * Math.pow(0.75, years.toDouble())
            return inflation * supplyRaw / totalStakedRaw * 100.0
        }
    }
    ...
    fun decode(userStake: ByteArray, config: ByteArray): Position? {
        ...
        return Position(guardian, shares, raw.min(BigInteger.valueOf(Long.MAX_VALUE)).toLong(), total.min(BigInteger.valueOf(Long.MAX_VALUE)).toLong())
    }
